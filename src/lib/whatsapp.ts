...
export type DatosPedidoMensaje = {
  ...
  // Monto descontado, transferencia + cupón combinados (0 = sin descuento).
  descuento?: number
  // Código de cupón usado, si usó uno (Fase 8e).
...
export function construirMensajePedido(d: DatosPedidoMensaje): string {
  ...
  if (d.descuento && d.descuento > 0) {
    const detalle = d.cuponCodigo ? ` (incluye cupón ${d.cuponCodigo})` : ''
    lineas.push(`Descuento: -${formatARS(d.descuento)}${detalle}`)
  }
